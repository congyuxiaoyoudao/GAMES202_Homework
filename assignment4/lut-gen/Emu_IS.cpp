...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    Vec3f up = abs(N.z) < 0.999 ? Vec3f(0.0, 0.0, 1.0) : Vec3f(1.0, 0.0, 0.0); // N is fixed 001 so not select 001
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    float Emu = 0.0;
    // split sum
    float Emu_split1 = 0.0;
    float Emu_split2 = 0.0;
    for (int i = 0; i < sample_count; i++) {
        ...
        // calculate (fr * ni) / p_o here - Bonus 1
        float w = VoH * GeometrySmith(roughness,NoV,NoL)/(NoV*NoH);
        Emu += w;
        // Split Sum - Bonus 2
    ...
    return Vec3f(Emu / sample_count, Emu_split1 / sample_count, Emu_split2 / sample_count);
}
